...
cart_quantity_change_check = dispatch.Signal()
cart_quantity_change_check.__doc___ = """
Sent to check if it is possible to change item's quantity to new value.
Receivers get ``variant``, ``old_qty``, ``new_qty`` and ``result`` kwargs.
``result`` may receive a tuple ``(result_quantity, reason)`` where reason is a
